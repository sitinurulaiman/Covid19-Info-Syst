...
$select="select * from admin_login where user_name='$user_name' and password='".md5($password)."'";
$res=mysqli_query($con,$select);
$data=mysqli_fetch_array($res);
if($data['user_name']!=$user_name or $data['password']!=md5($password))
{
    $_SESSION['error']='ok';
    ?>   <script type="text/javascript">window.location.href="login.php"</script>   <?php
}

elseif($data['user_name']==$user_name and $data['password']==md5($password))
{
